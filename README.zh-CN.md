...
- **阶段 4.3 ✅**：媒体消息 —— 5 个类型化发送方法（图片/视频/音频/文档/贴纸）+ 入站 `MediaInfo` + Python 与 Go sidecar 之间的流式上传/下载。
- **CI 已验证**：多平台 wheel 包已通过自动化流水线测试（mypy strict、ruff、244 项测试）。
- **分发**：已发布到 [PyPI](https://pypi.org/project/fastmeow/)（最新版本：`0.3.1`）。
...
- **v0.3.1 ✅**：阶段 4.3 已发布到 PyPI（针对 `0.3.0` 协议号问题的 hotfix）。
- **下一步**：
  - 阶段 5：表情反应、消息编辑、撤回、delete-for-me 通知（v0.4.0）。
- **远期计划**：
  - 高级会话管理 UI。
  - Worker / Broker 部署拓扑。
